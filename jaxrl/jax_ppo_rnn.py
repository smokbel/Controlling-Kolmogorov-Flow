...
import distrax
import flow_env_gymnax_stabilization
import gymnax
import functools
from gymnax.environments import spaces
from wrappers import FlattenObservationWrapper, LogWrapper, ClipAction, VecEnv
...
class ActorCriticRNN(nn.Module):
    ...
    def __call__(self, hidden, x):
        ...
        # if len(actor_mean.shape) == 3:
        #     actor_mean = jnp.squeeze(actor_mean, axis=0)
...
def make_train(config):
    ...
    # env = flow_env_gymnax_stabilization.flow
    # env, env_params = gymnax.make(config["ENV_NAME"])
    env = flow_env_gymnax_stabilization.KolmogorovFlow()
    ...
    def train(rng):
        # INIT NETWORK
        network = ActorCriticRNN(*env.action_space(env_params).shape, config=config)
...
if __name__ == "__main__":
    ...
    save_model(trained_params, 'trained_model_stabilization_rnn_oct6.pkl')
    
    plt.plot(out["metrics"]["returned_episode_returns"].mean(-1).reshape(-1))
    plt.xlabel("Updates")
    plt.ylabel("Return")
    plt.show() 
    plt.savefig("plot_stabilization.png", format='png')
